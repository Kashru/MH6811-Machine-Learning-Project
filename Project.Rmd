...
```{r}
#Train with Decision Tree selected above using full training set
train2 <- train[,c(2:13)]
train2$Gender <- as.factor(train2$Gender)
train2$Married <- as.factor(train2$Married)
train2$Dependents <- as.factor(train2$Dependents)
train2$Education <- as.factor(train2$Education)
train2$Self_Employed <- as.factor(train2$Self_Employed)
train2$Property_Area <- as.factor(train2$Property_Area)
train2$Loan_Status <- as.factor(train2$Loan_Status)
train2$Credit_History <- as.factor(train2$Credit_History)
test2 <- test[, c(2:12)]
...
set.seed(seed)
loan.tree.final <- tree(Loan_Status~.,data = train2)
loan.tree.final.pruned=prune.tree(loan.tree.final, best=loan.tree.bestsize) 
summary(loan.tree.final.pruned)
plot(loan.tree.final.pruned)
text(loan.tree.final.pruned)
loan.tree.pred.final <- predict(loan.tree.final.pruned, test2, type = "class")
...
```{r}
#Train with RandomForest using all training data
set.seed(seed)
p = ncol(train2)-1
loan.rf.final = randomForest(Loan_Status~., data = train2, mtry = ceiling(sqrt(p)),
  ntree = 500, importance = TRUE)
loan.rf.final
importance(loan.rf.final)
varImpPlot(loan.rf.final)
loan.rf.pred.final <- predict(loan.rf.final, newdata = test2, type = "class")
#End of RandomForest
```
...
```{r}
#Train with AdaBoost using all training data
n.train.final <- nrow(train2)
n.test.final <- nrow(test2)
train2.X=model.matrix(Loan_Status~.-1,data=train2) 
train2.y.ada=rep(1,n.train.final); train2.y.ada[train2$Loan_Status=="N"]=-1  
test2.X=model.matrix(~.-1, data=test2) 
set.seed(seed)
loan.ada.final = adaboost(train2.X, train2.y.ada, n_rounds = 500)
loan.ada.predict.raw = predict(loan.ada.final, test2.X)
...
```{r}
#Train with XGBoost using all training data
train2.y.xgb=rep(1,n.train.final); train2.y.xgb[train2$Loan_Status=="N"]=0  
test2.y.xgb=rep(1,n.test.final)
set.seed(seed)
loan.xgb.final = xgboost(data = train2.X, label = train2.y.xgb, nrounds = 500, verbose = FALSE)
loan.xgb.pred.raw = predict(loan.xgb.final, test2.X)
